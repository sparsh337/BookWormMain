    ...
    public InvoiceResponseDTO generateInvoice(Integer userId) {
        ...
        for (Cart item : cartItems) {
            ...
            // ===== LIBRARY (MEMBERSHIP) =====
                ...
                shelfService.lendFromSubscription(item.getCustomer().getUserId(), product.getProductId());

                detail.setTranType('L');
                detail.setQuantity(1);
                detail.setBasePrice(BigDecimal.ZERO);
                detail.setSalePrice(BigDecimal.ZERO);
                detail.setRentNoOfDays(0);

                // No cost added to totalAmount
            }
            // ===== PURCHASE =====
            else {
        ...
        // (optional, just for confirmation during testing)
        System.out.println("Invoice PDF generated, size = " + pdfBytes.length);
