...
const ProductList = () => {
    ...
    const [maxLibraryBooks, setMaxLibraryBooks] = useState(0);
    const { user } = useAuth();
        ...
        const fetchShelf = async () => {
            if (user?.userId) {
                try {
                    ...
                    // Fetch Active Subscription if in library mode
                    if (mode === 'library') {
                        const subRes = await api.get(`/api/subscriptions/active/${user.userId}`);
                        if (subRes.data && subRes.data.libraryPackage) {
                            setMaxLibraryBooks(subRes.data.libraryPackage.maxSelectableBooks);
                        }
                    }
                } catch (e) {
                    console.error("Failed to fetch shelf/subscription", e);
                }
            }
        };
        fetchShelf();
    ...
    const handleLibraryAction = async (product) => {
        // Check Limit
        const currentCount = ownedItems.filter(i => i.tranType === 'L').length;
        if (currentCount >= maxLibraryBooks) {
            toast.warning(`You have reached your limit of ${maxLibraryBooks} books.`);
            return;
        }

        try {
            await api.post('/api/library/lend', {
                userId: user.userId,
                productId: product.productId,
                tranType: 'L'
            });
            toast.success("Added to Library!");
            // Refresh Shelf to update ownership status
            if (user?.userId) {
                const [shelfRes, libraryRes] = await Promise.all([
                    api.get(`/api/shelf/${user.userId}`),
                    api.get(`/api/library/${user.userId}`)
    ...
    useEffect(() => {
        document.title = mode === 'library' ? "Digital Library | BookWorm" : "Browse Books | BookWorm";
        // Fetch Metadata
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            ...
            try {
                ...
                if (selectedGenres.length > 0 || selectedLanguages.length > 0 || selectedTypes.length > 0) {
                    ...
                    if (selectedTypes.length > 0) params.append('typeIds', selectedTypes.join(','));
        ...
        fetchProducts();
    }, [selectedGenres, selectedLanguages, selectedTypes, mode, search]);
        ...
        <div className="min-h-screen bg-body pb-20">
            ...
            <div className="container py-8">
             ...
             `}>
                        ...
                        <div className="border-t pt-6">
                            <h3 className="font-semibold mb-3">Formats</h3>
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 cursor-pointer hover:text-accent">
                                    <input
                                        type="checkbox"
                                ...
                                {types.map(t => (
                                    <label key={t.typeId} className="flex items-center gap-2 cursor-pointer hover:text-accent">
                                        <input
                                            type="checkbox"
                                            value={t.typeId}
                                            checked={selectedTypes.includes(t.typeId)}
                                            onChange={() => toggleType(t.typeId)}
                                            className="accent-accent"
                                        />
                                        <span>{t.typeDesc}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="flex-1">
                        {loading ? (
                                ...
                                {products.map(product => {
                                    ...
                                    return (
                                        <ProductCard
                                            ...
                                            mode={mode}
                                            ownedStatus={owned}
                                            onLibraryAction={() => handleLibraryAction(product)}
                                        />
                                    );
                                })}
                            </div>
                            ...
                            <div className="text-center py-20 bg-white rounded-lg border border-dashed border-border">
                                ...
                                <button
                                    onClick={() => { setSelectedGenres([]); setSelectedLanguages([]); setSelectedTypes([]) }}
            ...
            {mode === 'library' && (
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-border shadow-lg p-4 animate-slide-up z-50">
                    <div className="container flex justify-between items-center">
                        <div>
                            <span className="text-gray-600">Library Usage:</span>
                            <span className="text-2xl font-bold text-indigo-600 ml-2">
                                {ownedItems.filter(i => i.tranType === 'L').length} / {maxLibraryBooks}
                            </span>
                        </div>
                        <button
                            onClick={() => navigate('/library')}
                            className="btn btn-secondary px-8 py-3 rounded-full shadow-lg flex items-center gap-2 text-lg hover:scale-105 transition-transform"
                        >
                            Go to My Library <ArrowRight size={20} />
                        </button>
                    </div>
                </div>
            )}
