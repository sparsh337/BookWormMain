...
import { BookOpen, Clock, Calendar, AlertCircle, Trash2, X, Lock } from 'lucide-react';
...
const MyShelf = () => {
    ...
    useEffect(() => {
        document.title = "My Shelf | BookWorm";
        if (user) {
            fetchShelf();
        }
    }, [user]);
    ...
    const getTypeLabel = (type) => {
        switch (type) {
            case 'P': return { label: 'Purchased', color: 'bg-green-100 text-green-700 text-center' };
            case 'R': return { label: 'Rented', color: 'bg-orange-500 text-white shadow-sm text-center' };
            case 'L': return { label: 'Library', color: 'bg-indigo-100 text-indigo-700 text-center' };
            default: return { label: 'Unknown', color: 'bg-gray-100 text-gray-700 text-center' };
        }
    };
                        ...
                        {mediaLoading ? (
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'white', padding: '40px' }}>
                                <div style={{ textAlign: 'center', padding: '50px', backgroundColor: '#0f172a', borderRadius: '28px', border: '1px solid #1e293b', maxWidth: '380px', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
                                    ...
                                    <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: '1.6' }}>Secure connection interrupted. Please return to your shelf and try again.</p>
                                    <button onClick={closeReader} style={{ marginTop: '30px', width: '100%', padding: '14px', backgroundColor: '#6366f1', color: 'white', border: 'none', borderRadius: '14px', fontWeight: '900', textTransform: 'uppercase', cursor: 'pointer', fontSize: '13px', letterSpacing: '0.1em' }}>Back to Shelf</button>
                                </div>
                            </div>
                        )}
                    </main>
                                        ...
                                        <div className="p-4 flex flex-col h-full">
                                            ...
                                            <div className="flex-1 flex flex-col">
                                                ...
                                                <div className="flex justify-center mb-4">
                                                    ...
                                                    {(!item.productExpiryDate || item.tranType === 'P') && (
                                                        <div className="flex items-center gap-1 text-xs text-green-600 bg-green-50 px-2 py-1 rounded-full inline-flex">
                                                            <Calendar size={12} />
                                                ...
                                                <div className="flex gap-2 mt-auto pt-2 border-t border-gray-50 justify-center">
                                                    <button
                                                        ...
                                                        className="btn btn-primary btn-sm flex items-center justify-center gap-2 text-xs py-2 px-10 flex-none"
                                                    >
                                                    ...
                                                    {item.tranType === 'R' && (
                                                        <button
                                                            ...
                                                            className="btn btn-sm flex-shrink-0 bg-red-600 text-white border border-red-600 p-2 rounded-lg hover:bg-red-700 transition-all shadow-sm"
