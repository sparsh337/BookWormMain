...
public class ShelfService {
    ...
    public List<MyShelf> getmylibrary(int userId) {
        ...
        // Physically remove expired items from DB
        List<MyShelf> expired = list.stream()
                .filter(item -> item.getProductExpiryDate() != null &&
                        item.getProductExpiryDate().isBefore(today))
                .toList();

        if (!expired.isEmpty()) {
            shelfrepos.deleteAll(expired);
        }

        // Return only the remaining active items
        return shelfrepos.findByUserIdAndTranTypeIn(userId, List.of('R', 'L'));
    }
    ...
    public List<MyShelf> getAllLibraryItems() {
        LocalDate today = LocalDate.now();
        List<MyShelf> allLibrary = shelfrepos.findByTranTypeIn(List.of('R', 'L'));

        List<MyShelf> expired = allLibrary.stream()
                .filter(item -> item.getProductExpiryDate() != null && item.getProductExpiryDate().isBefore(today))
                .toList();
        if (!expired.isEmpty()) {
            shelfrepos.deleteAll(expired);
        }
    ...
    public MyShelf addToShelf(int productId, int userId, char tranType, Integer rentDays) {
        ...
        if (tranType == 'P') {
            ...
            royaltyTransactionService.generateRoyalty(userId, product, 'P', purchaseAmount, purchaseAmount, 1, 0, null);
            ...
            royaltyTransactionService.generateRoyalty(userId, product, 'R', rentAmount, product.getRentPerDay(), 1,
                    rentDays, null);
        }
    ...
    public MyShelf lendFromSubscription(int userId, int productId) {
        return lendFromSubscription(userId, productId, null);
    }

    public MyShelf lendFromSubscription(int userId, int productId, com.example.model.Invoice invoice) {
        ...
        if (today.isAfter(subscription.getEndDate())) {
            ...
            // Clear books added via this subscription
            shelfrepos.deleteByUserIdAndTranType(userId, 'L');
        ...
        // Calculate rent days for royalty transaction (days remaining in subscription)
        int rentDays = (int) java.time.temporal.ChronoUnit.DAYS.between(today, subscription.getEndDate());

        // ✅ Generate royalty for L (subscription)
        royaltyTransactionService.generateRoyalty(userId, product, 'L', perBookValue, perBookValue, 1, rentDays,
                invoice);
